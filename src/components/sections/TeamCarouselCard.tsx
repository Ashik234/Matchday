...
import { CardFlagBackdrop } from '@/components/ui/CardFlagBackdrop';
...
export function TeamCarouselCard({ team }: { team: Team }) {
  return (
    <Link to={`/team/${toSlug(team.name)}`} className="block">
      <CardFlagBackdrop countryCode={team.countryCode} className="rounded-xl">
        <Card hover className="w-[260px] shrink-0 snap-start flex flex-col bg-bg-elev1/80 backdrop-blur-sm">
